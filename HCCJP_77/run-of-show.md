...
> **配信で見せるのは L1 の Hyper-V マネージャー**。L0 に NAT を1本足したので、
> `mstsc /v:192.168.1.253:13389` で **1回のRDPで直接 L1 に入れる**（実測OK）。
> ユーザー `Administrator`、**資格情報は事前に保存しておく**（本番でパスワードを打たない）。
>
> ### 巻き戻しのGUI操作（4手）
>
> 1. `arcwin01` を選ぶ → 下の［チェックポイント］で **`T1-hccjp77` を右クリック →［適用］**
> 2. ダイアログは **［適用］**（［チェックポイントの作成と適用］は選ばない）
> 3. **⚠️ VMが「オフ」になる** ─ 4つのチェックポイントは全て `State=Off` で保存されているため、
>    **適用しただけでは起動しない**
> 4. **`arcwin01` を右クリック →［起動］** → 「実行中」を確認して**時刻を口で宣言**
>
> 保険として、L0 の PowerShell から実行できるスクリプトも
> `C:\hccjp77\` に置いてある（パスワードはファイル内。配信画面に映らない）。
>
> | スクリプト | 何をするか | 所要 |
> |---|---|---|
> | `C:\hccjp77\1-status.ps1` | チェックポイント一覧＋**実機が持っている割り当て** | 約20秒 |
> | `C:\hccjp77\2-rollback.ps1` | 巻き戻しの**保険**（GUIが固まったとき用・起動まで自動） | 約12秒 |
...
| # | やること | 理由 |
|---|---|---|
| 0 | **`mstsc /v:192.168.1.253:13389` で L1 に RDP し、Hyper-V マネージャーを開いたまま置く** | arcwin01 の［チェックポイント］に `T1-hccjp77` と `T6-demo-ready` が見えることを確認する。**無ければデモが成立しない** |
...
### 見せるのは L1 の Hyper-V マネージャー

`mstsc /v:192.168.1.253:13389` で入った L1 の画面で、**適用 → 起動**の2ステップ。

1. `arcwin01` → ［チェックポイント］→ `T1-hccjp77` を右クリック → **［適用］**
2. ダイアログは **［適用］**
3. **VMが「オフ」になる**（チェックポイントは `State=Off` で保存されている）
4. `arcwin01` を右クリック → **［起動］**
5. 「実行中」を確認して、**時刻を口で宣言する**

保険は L0 の `C:\hccjp77\2-rollback.ps1`（起動まで自動・約12秒）。**GUIで詰まったら深追いしない。**
...
Hyper-V マネージャーで **`T6-demo-ready` を［適用］→ arcwin01 を［起動］**。またはL0で:
